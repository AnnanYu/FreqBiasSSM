...
from models.s4.s4_PTD import S4_PTD
...
class S4Model(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            self.s4_layers.append(
                S4_PTD(d_model, dropout=dropout, transposed=True, lr=min(0.001, args.lr))
            )
    ...
    def forward(self, x, noisy):
        ...
        s = x.shape
        #no = torch.zeros(1,device=x.device)
        for layer, norm, dropout in zip(self.s4_layers, self.norms, self.dropouts):
            ...
            if noisy:
                noise = torch.rand((s[0],s[1],1), dtype=z.dtype, device=z.device)
                noise = noise.repeat(1,1,s[2])
                dt = torch.exp(layer.kernel.log_dt).detach().cpu().numpy()
                wave = torch.zeros((1,s[1],s[2]), dtype=z.dtype, device=z.device)
                for i in range(s[1]):
                    wave[0,i,:] = torch.cos(325.4*torch.linspace(0, dt[i]*(s[2]-1), s[2]))
                wave = wave.repeat(s[0],1,1)
                noise *= wave
                noise = 1e-1 * noise
            else:
                noise = torch.zeros(s, dtype=z.dtype, device=z.device)
...
def train():
    ...
    for batch_idx, (inputs, targets) in pbar:
        ...
        optimizer.zero_grad()
        outputs, no = model(inputs,False)
        loss = criterion(outputs, targets) #+ 2 * no
        loss.backward()
        optimizer.step()
...
def eval(epoch, dataloader, noise, checkpoint=False):
    global best_acc
    model.eval()
    eval_loss = 0
    correct = 0
...
if __name__ == '__main__':
    ...
    for epoch in pbar:
        ...
        train_acc = train()
        val_acc = eval(epoch, valloader, False, checkpoint=True)
        test_acc = eval(epoch, testloader, True)
        acc_train = np.append(acc_train, train_acc)
        acc_val = np.append(acc_val, val_acc)
        acc_test = np.append(acc_test, test_acc)
        scheduler.step()
        # print(f"Epoch {epoch} learning rate: {scheduler.get_last_lr()}")
    dic = {'acc_train':acc_train, 'acc_val':acc_val, 'acc_test':acc_test}
    mlio.savemat('acc_S4P_inn3.mat',dic)
